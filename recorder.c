...
void* recorder_init(char *filename, AVFormatContext *ifc)
{
    ...
    for (i = 0; i < ifc->nb_streams; i++) {
        ...
        AVStream *os = avformat_new_stream(recorder->ofc, NULL);
        if (!os) {
...
int recorder_packet(void *ctxt, AVPacket *pkt)
{
    ...
    if (!ctxt || !pkt || pkt->stream_index >= recorder->ifc->nb_streams || recorder->stream_mapping[pkt->stream_index] < 0) return -1;
